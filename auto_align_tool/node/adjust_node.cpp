...
static pcl::PointCloud<pcl::PointXYZI> g_cloud_;

static std::vector<float> trans_vec_;
static std::vector<float> car_vec_;

static pcl::PCLHeader header_;
...
{
  header_ = msg->header;
  std::cout << "receive a new frame!" << std::endl;
  g_cloud_ = *msg;
  pushlishAll();
}
...
int main(int argc, char *argv[])
{
  ...
  pub_trans_frame_ = node.advertise<pcl::PointCloud<pcl::PointXYZI> >("/adjust", 1);
  ...
  std::string tmp_save_path = ros::package::getPath("auto_align_tool") + "/save_config/";
...
//    final_trans_vec[0] += lidar_vec_[0];
//    final_trans_vec[1] += lidar_vec_[1];
//    final_trans_vec[2] += (car_vec_[2] + lidar_vec_[2]);
  ...
  transfor_cloud_ptr->header = header_;
  pub_trans_frame_.publish(transfor_cloud_ptr);
}
...
void publishDirection_x()
{
  ...
  tf::Quaternion quat = tf::createQuaternionFromRPY(trans_vec_[3], trans_vec_[4], trans_vec_[5]);
